...
def gerar_etiqueta_pequena(codigo: str) -> Image.Image:
    """Gera imagem de etiqueta individual com QR code e código embaixo."""
    ...
    try:
        ...
        tw = len(codigo) * 36  # fallback para versões antigas
...
def gerar_etiqueta_larga(codigos: list[str]) -> Image.Image:
    """Gera etiqueta larga com até 7 QR codes lado a lado."""
    ...
    # Título baseado no primeiro código (formato RUA.POSICAO)
    if codigos and "." in str(codigos[0]):
        partes = str(codigos[0]).split(".")
...
def gerar_pdf_lote(codigos: list[str]) -> bytes:
    """Gera PDF com uma etiqueta por página (formato 65x100mm)."""
    pdf = FPDF("L", "mm", (65, 100))
    for codigo in codigos:
        img = gerar_etiqueta_pequena(codigo)
        img_bytes = _imagem_para_bytes(img)
        pdf.add_page()
        # Salva temporariamente em memória via BytesIO
...
def gerar_pdf_larga(grupos: list[list[str]]) -> bytes:
    """Gera PDF de etiquetas largas (315x80mm), um grupo por página."""
