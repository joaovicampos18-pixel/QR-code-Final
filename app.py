...
app.py - Interface Streamlit (apenas UI, sem logica de negocio)
...
@st.cache_resource
 ...
 st.error("Nao foi possivel conectar ao banco. Verifique o arquivo .env")
...
px = st.session_state.proximo
 ...
 st.metric("Proximo Codigo", f"{px:08d}")
...
with t1:
 ...
 st.image(gerar_etiqueta_pequena(f"{px:08d}"), width=400, caption="Previa do primeiro codigo")
 ...
 st.warning("PDF gerado, mas nao foi possivel registrar no banco.")
...
with t2:
 ...
 codigo_manual = st.text_input("Digite o codigo:", key="manual_input")

 if codigo_manual:

 st.image(gerar_etiqueta_pequena(codigo_manual), width=400, caption="Previa")
...
with t3:
 ...
 st.caption("Cole um codigo por linha")

 lista_raw = st.text_area("Lista de codigos:", height=200, key="lista_input")

 codigos_lista = [c.strip() for c in lista_raw.splitlines() if c.strip()]

 if codigos_lista:

st.info(f"{len(codigos_lista)} codigo(s) detectado(s)")

 st.image(gerar_etiqueta_pequena(codigos_lista[0]), width=400, caption="Previa do primeiro")
...
with t4:
 ...
 st.caption("Cole os codigos (7 por folha, um por linha)")

 larga_raw = st.text_area("Codigos:", height=200, key="larga_input")
...
st.info(f"{len(itens_larga)} codigo(s) -> {len(grupos)} folha(s)")

 st.image(gerar_etiqueta_larga(grupos[0]), use_container_width=True, caption="Previa da primeira folha")
